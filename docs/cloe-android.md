...
**消息格式**：
```json
{"action":"speak","audio":"doing"}           // 预录语音（bridge /audio/ 路由）
{"action":"speak","audio_url":"http://..."}   // TTS 语音（bridge /tts/ 路由）
```

**音频源 URL 规则**：
- 预录：`http://<host>:19851/audio/<name>.mp3`（bridge serve `~/.cloe/audio/`）
- TTS：`http://<host>:19851/tts/<filename>.mp3`（bridge serve `~/.cloe/audio_cache/`）

**⚠️ 跨设备 URL 替换**：`audio_url` 中的 `localhost`/`127.0.0.1` 必须替换为 `host`（安卓连接时配置的 PC IP）。`audioName` 模式直接拼接 `host`。

**⚠️ Bridge 必须有 `/audio/` 路由**：预录音频走 `GET /audio/:filename`（serve `~/.cloe/audio/`），TTS 走 `GET /tts/:filename`。如果 `/audio/` 返回 404，说明 bridge 版本太旧需更新 launcher.js。
...
**音频缓存**：下载到 `cacheDir/audio/`，文件存在则跳过下载（秒播）。
...
**调试**：
```bash
# 检查 bridge 是否 serve 预录音频
curl -s -o /dev/null -w "%{http_code}" http://localhost:19851/audio/doing.mp3
# 期望 200，如果是 404 说明 bridge 缺 /audio/ 路由

# 查看安卓端日志
adb logcat -d | grep CloeService
# 关注：Audio downloaded / Audio playing / Audio error / Failed to download
```

**⚠️ MediaPlayer 在 IO 线程下载、切 Main 线程播放**。`setDataSource` 必须用本地文件路径（`File.absolutePath`），不能直接用 URL。
